...
<div class="container">
		<div class="row">
			<div class="col-md-12 mt-5 pt-3">
				...
				<div class="float-left">
					<h2 class="ml-2 top-project-heading">{{ __('msg.projects')}}<h6 class="ml-2 total-results">50 results</h6></h2>
				</div>
			</div>
			<div class="col-md-12">
	...
	<div class="container mb-5">
		<div class="row">
			...
			<a href="javascript:{}" onclick="document.getElementById('form_{{$project->id}}').submit();" >
			<div class="col-md-4 mt-4 pt-1 pd-left">
				<div class="card border-light rounded-xl shadow-sm">
					...
					<div class="card-body project-title p-3">
						<div class="bordr-botm">
							<h5 class="card-title"><span class="project-name">{{$project->title}}</span>  <span class="float-right">
								<a href="{{ url('delete-project/'.$project->id) }}"><img src="assets/images/delete.svg"></a>
							</span></h5>
						</div>
						<div class="card-text m-1">
							...
							<div class="mt-2">
								...
								<span class="r-date ml-1">{{ __('msg.privacy')}} </span> <span class="p-status float-right"> {{ __('msg.private')}}</span>
							</div>
						</div>
						<form action="{{ route('projectInfo')}}" method="post" id="form_{{$project->id}}">{{ csrf_field() }}
