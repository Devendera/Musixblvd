...
/* Function for return view for project information page */
    public function projectInfo(Request $request)
    {
        ...
        $roles = \DB::table('roles')->get();
        return view('web.projectinfo', compact('roles', 'detail', 'contributors', 'platforms', 'requestConnections', 'message'));
    }
...
/* Function for return view for managers page */
    ...
    public function studioMapView(Request $request)
    {
        ...
        return view('web.studio_map_view', compact('studioJsonData'));
    }
    ...
    public function studioProfile(Request $request)
    {
        ...
        return view('web.studio_profile', compact('userData', 'userImg', 'profileImg', 'projectData', 'numberOfConnections', 'numberOfProjects'));
    }
...
/* Function for return view for managers profile page */
    public function managerProfile(Request $request)
    {
        ...
        return view('web.manager_profile', compact('userData', 'nameArr', 'numberOfConnections', 'reciever', 'sender', 'numberOfClients'));
    }
...
/* Function for return view for managers view profile page for not login user */
    public function managerViewProfile(Request $request, $id)
    {
        ...
        return view('web.manager_profile_view', compact('sender', 'reciever', 'userData', 'status', 'request', 'numberOfConnections', 'numberOfClients'));
    }
...
/* Function for return view for managers view profile page for not login user */
    public function studioViewProfile(Request $request, $id)
    {
        ...
        return view('web.studio_profile_view', compact('userData', 'numberOfConnections', 'numberOfProjects', 'userImg', 'profileImg', 'projectData', 'status'));
    }
...
/* Function for return view for creative view profile page for not login user */
    ...
    {$clientData = [];
        $user = Auth::guard('web')->user();
        $userId = $user->id;
        $userData = \DB::table('users')->select('users.*', 'creatives.type as creativesType', 'creatives.genre as creativesGenre', 'creatives.Craft as creativesCraft', 'creatives.Gender as creativesGnder', 'creatives.website as website', 'creatives.stage as stage', 'creatives.pro as pro', 'creatives.status as status')->join('creatives', 'users.id', '=', 'creatives.user_id')->where('users.id', '=', $id)->first();
        $connection = \DB::table('connections')
            ->where([['connected_user_id', '=', $id], ['user_id', '=', $userId]])
            ->first();
        ...
        return view('web.creative_profile_view', compact('reciever', 'sender', 'numberOfConnections', 'numberOfProjects', 'userData', 'status', 'projectData', 'genres', 'genders', 'types', 'crafts', 'platforms'));
    }
    ...
    public function userIndex(Request $request)
    {
        ...
        if (count($users) > 0) {
            return view('web.user_index', compact('streamingData','users', 'genres', 'genders', 'crafts', 'types', 'countries', 'searchGender', 'searchGenre', 'searchType', 'searchState', 'searchCountry', 'searchArtistry', 'searchCity', 'searchConnection', 'searchProject', 'searchQuery'));
        } else {
            return view('web.user_index', compact('streamingData','users', 'genres', 'genders', 'crafts', 'types', 'countries', 'genres', 'genders', 'crafts', 'types', 'countries', 'searchGender', 'searchGenre', 'searchType', 'searchState', 'searchCountry', 'searchArtistry', 'searchCity', 'searchConnection', 'searchProject', 'searchQuery'))->withErrors(config('constants.MatchRecord'));
        }
    }
...
/* Function for return view for edit manager profile page */
    ...
    public function updateStudioProfile(Request $request)
    {
        ...
        $numberOfProjects = UserProject::
            where([['user_id', '=', $userId]])->count();
        return view('web.studio_profile', compact('userData', 'userImg', 'profileImg','numberOfConnections','numberOfProjects','projectData'));
...
/* Function for register new manager in web based */
    public function registerManager(Request $request)
    {
        ...
        $manager->save();
        return redirect('/user-index')->with('flash_message_success', Config::get('constants.ManagerRegister'));
    }
    ...
    public function registerStudio(Request $request)
    {
        ...
        return redirect('/user-index')->with('flash_message_success', Config::get('constants.StudioRegister'));
    }
    ...
    public function registerCreative(Request $request)
    {
        if (Auth::guard('web')->check()) {
            ...
            if ($user->has_profile == 1) {
                ...
                return redirect('/user-index')->with('flash_message_success', config('constants.ManagerRegister'));
            }
        } else {
        ...
        if (Auth::guard($this->guard)->check()) {
            ...
            $sender = \DB::table('connections')
                ...
                ->where([
                    ['connections.user_id', '=', $userId],
                    ['connections.is_approved', '=', '1'],
                    ['connections.type', '=', Config::get('constants.management')]])
                ->get();
            $reciever = \DB::table('connections')
                ->select('users.id', 'users.name', 'users.img', 'users.email')
                ->Rightjoin('users', 'connections.user_id', '=', 'users.id')
            ...
            $projectData = \DB::table('projects')
                ...
                ->where('user_projects.user_id', '=', $userId)
                ->get();
            return view('web.profile', compact('projectData', 'reciever', 'sender', 'genres', 'genders', 'crafts', 'types', 'platforms', 'user', 'requestConnections'));
...
/****************Update Creative profile data******/
    ...
    public function sendConnectionRequest($userId, $contributorId)
    {
        ...
        $connectionSender->type = Config::get('constants.project');
    ...
    public function addConnection(Request $request, $id)
    {
        ...
        if ($connectionId) {
            $updateFlag = \DB::table('project_edit_requests')->where([['project_id', '=', $connectionId->project_id], ['user_id', '=', $connectionId->user_id], ['contributor_id', '=', $connectionId->contributor_id]])->update($values);
        }
...
/* Function for delete project for user */
    ...
    public function getStreaming(Request $request)
    {
        ...
        $mediaProvider->save();
        return redirect('/user-index')->with('flash_message_success', Config::get('ChannelAdded'));     
        }
    } 
        }
        catch(customException $e){
        
            return redirect('/user-index')->with('flash_message_error', Config::get('Invalid API key or channel ID'));
        } 
    }
    public function getSpotify(Request $request){
        ...
        $mediaProvider->save();
        return redirect('/user-index')->with('flash_message_success', Config::get('ChannelAdded'));     
    } else {return redirect('/user-index')->with('flash_message_success', Config::get('ChannelNotAdded'));
    }
}     
    /* For delete stream */
    public function deleteStreaming(Request $request,$id){
        mediaProvider::where('id', $id)->delete();
        return redirect('/user-index')->with('flash_message_success', Config::get('streamdelete'));   
    }
}
